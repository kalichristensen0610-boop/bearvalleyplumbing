...
export function Header(){
  ...
  return <>
    ...
    <header className={headerHidden?"site-header header-hidden":"site-header"}>
      <Link href="/" className="brand" aria-label="Bear River Plumbing home" onClick={closeMenus}><Image src="/bear-river-logo-transparent.png" width={1672} height={941} alt="Bear River Plumbing LLC" priority sizes="(max-width: 760px) 132px, 170px" /></Link>
      ...
      <nav id="primary-nav" className={open?"nav open":"nav"} aria-label="Primary navigation">
        ...
        <div className={activeMenu==="services"?"nav-group expanded":"nav-group"}>
          ...
          <div id="services-menu" className="dropdown services-dropdown"><div><p>Explore</p><Link onClick={closeMenus} href="/services">All Services</Link><Link onClick={closeMenus} href="/residential-plumbing">Residential Plumbing</Link></div><div><p>Plumbing services</p>{services.slice(0,3).map(s=><Link onClick={closeMenus} key={s.slug} href={`/services/${s.slug}`}>{s.name}</Link>)}</div><div><p>More services</p>{services.slice(3).map(s=><Link onClick={closeMenus} key={s.slug} href={`/services/${s.slug}`}>{s.name}</Link>)}</div></div>
        </div>
